...
@section('title', 'Edit Seeds Produk')

@section('main')
    <div class="content">
        <form class="mb-9" action="{{ route('seeds.update', $seeds->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row g-3 flex-between-end mb-5">
                <div class="col-auto">
                    <h2 class="mb-2">Edit Seeds Produk</h2>
                    <h5 class="text-700 fw-semi-bold">Silakan lengkapi form di bawah ini untuk mengedit produk.</h5>
                </div>
                <div class="col-auto">
                    <a class="btn btn-phoenix-secondary me-2 mb-2 mb-sm-0" href="{{ route('seeds') }}">Batalkan</a>
                ...
                <div class="col-12 col-xl-8">
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('nama_produk') is-invalid @enderror" id="namaProduk"
                            ...
                            value="{{ old('nama_produk', $seeds->nama_produk) }}" />
                        @error('nama_produk')
                    ...
                    <div class="mb-6">
                        ...
                        <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsiProduk" name="deskripsi"
                            rows="5" placeholder="Masukkan Deskripsi Produk">{{ old('deskripsi', $seeds->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <strong class="invalid-feedback">{{ $message }}</strong>
                        @enderror
                    </div>
                    <div class="mb-5">
                        ...
                        <strong class="invalid-feedback" id="image-error" style="display: none;">Inputan ini harus berupa
                            gambar</strong>
                        @if ($seeds->foto_produk)
                            <img class="mt-2" id="current-image"
                                src="{{ asset('storage/product/' . $seeds->foto_produk) }}" alt="Current Image"
                                                    ...
                                                    @else
                                                        @foreach ($kategori as $kategoris)
                                                            <option value="{{ $kategoris->id }}"
                                                                {{ old('kategori_id', $seeds->kategori_id) == $kategoris->id ? 'selected' : '' }}>
                                        ...
                                        <div class="col-12 col-sm-6 col-xl-12">
                                            <div class="mb-4">
                                                ...
                                                <input class="form-control @error('harga') is-invalid @enderror"
                                                    ...
                                                    name="harga" value="{{ old('harga', $seeds->harga) }}" />
                                                @error('harga')
                                                    <strong class="invalid-feedback">{{ $message }}</strong>
                                                @enderror
                                            </div>
